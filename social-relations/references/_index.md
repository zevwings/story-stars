...
- package 安装的 `protocols/` 是**方法规范层**: 定义社会关系场的建模、校验与写作边界；项目自行建立且已经确认的 `mechanisms/` 才承载本项目长期机制事实。
- 本目录是**参考层**: 除本 `_index.md` 与 `library/` managed subtree 外只放项目自有词条与写法素材，`library/` 只放 package-managed 通用素材；两者都**不构成任何事实，不规定任何人物的偏好与经历**。
- 参考与协议冲突时服从 `protocols/`；参考与项目正式机制冲突时服从项目 `mechanisms/` 及更高层正式真源。

## 读取规则

- 先按 `protocols/_index.md` 读取本任务命中的方法页；项目存在已确认机制时再读对应 `mechanisms/`，随后核对人物与其它正式真源，最后才来本目录取观察问题。
...
- 项目自有参考页应标注对应协议；若引用项目长期机制，再附对应 `mechanisms/` 正式路径。
