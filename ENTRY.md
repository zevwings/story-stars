...
- 故事事实、写作边界和作品级承诺，以 `BIBLE.md` 为最高真源；目录权限、候选落盘、施工和决策门禁以 `POLICY.md` 为准。
...
- `story/construction/`：已确认执行规格的入口。具体状态、继承范围和正式节点接口服从 `POLICY.md` 与本目录 `_status/`。
...
- 候选、缺口、延迟决策和已确认施工规格的具体落盘边界，统一服从 `POLICY.md`，本文不另行定义。
