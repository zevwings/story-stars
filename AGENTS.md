...
本文件只负责 Codex 启动、上述内容边界开关与强制策略入口，不定义故事事实、文风、目录职责或具体流程门禁。仓库级门禁以 `$PROJECT_POLICY` 的公共基线及项目扩展为准，故事事实与写作边界以 `$PROJECT_BIBLE` 为准，文风以 `$PROJECT_STYLE` 为准，读取顺序与任务路由以 `$PROJECT_ENTRY` 的公共路由及项目扩展为准。
